...
async function init() {
  ...
  const typeEffect = new TypeEffect(node);

  await sleep(1000);
  node.innerText = "";
  await typeEffect.type('Hello,\u00A0');

  while (true) {
    await typeEffect.type('Welcome');
    await sleep(2000);
    await typeEffect.delete('Welcome');
    await typeEffect.type('I\u00A0am\u00A0James');
    await sleep(2000);
    await typeEffect.delete('I\u00A0am\u00A0James');
  }
}
